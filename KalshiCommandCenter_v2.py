...
class KalshiDashboardV2(App):
    ...
    def _sync_worker(self):
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                self.call_from_thread(self._update_balance, f"⚠️ API Error: {res.status_code}")
    ...
    def start_bot(self, key: str):
        ...
        if key in self.bots and self.bots[key].poll() is None:
            self.log_message(f"⚠️ {info['name']} already running", "warning")
            return
        ...
        try:
            ...
            self.log_message(f"▶️ Started {info['name']} - {action}", "success")
    ...
    def stop_bot(self, key: str):
        ...
        if not proc:
            self.log_message(f"⚠️ {info['name']} not running", "warning")
            return
        
        try:
            proc.terminate()
            proc.wait(timeout=5)
            self.log_message(f"⏹️ Stopped {info['name']}", "success")
        except Exception:
            try:
                proc.kill()
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        # Tab controls
            ...
            self.log_message("⚠️ Showing errors only", "warning")
        elif bid == "btn_clear_logs":
            try:
                self.query_one("#filtered_log", Log).clear()
                self.log_message("🗑️ Logs cleared", "success")
            except Exception:
                pass
        ...
        try:
            ...
            for ticker in tickers:
                ...
                if res.status_code == 200:
                    ...
                    market_table.add_row(ticker, f"{yes_ask}¢", f"{no_ask}¢", f"{gap}¢", signal)
                else:
